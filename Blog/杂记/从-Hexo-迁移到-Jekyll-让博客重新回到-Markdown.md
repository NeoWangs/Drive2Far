...
## 如果想从头开始：Obsidian Jekyll Starter

这次迁移做完之后，我把其中比较通用的部分整理成了一个 starter 项目：[NeoWangs/obsidian-jekyll-starter](https://github.com/NeoWangs/obsidian-jekyll-starter)。

它不是把我这个博客主题原样打包出去，而是把这套写作和发布方式抽成一个更轻的起点：以 Obsidian 为优先编辑器，以 Markdown 作为唯一可信源，以 GitHub Actions 负责构建和部署。

starter 里已经包含了这些迁移时最容易反复踩的点：

- `_posts` 可以直接作为 Obsidian vault 打开。
- 文章文件名不需要 Jekyll 默认的日期前缀，发布日期放在 front matter 里。
- 文章图片放在同名素材目录，用本地相对路径引用，Obsidian 能预览，Jekyll 构建时会发布到文章 URL 下。
- `runcode` 使用普通 fenced code block，平时就是代码块，上线后变成可运行代码框。
- `pin`、`card`、`poem` 使用 Obsidian callout，编辑时像普通笔记，发布后变成样式化卡片。
- GitHub Pages 部署流程已经放在 `.github/workflows/` 里，push 后可以直接走 Actions。

如果只是想从零搭一个“Obsidian 写作 + Jekyll 生成 + GitHub Pages 发布”的博客，不一定要从旧 Hexo 站点一点点迁移。直接拿这个 starter 改 `_config.yml`、换布局和 CSS，会轻松很多。它也保留了很小的主题表面积，真正值得复用的是 `_plugins` 里那套 Obsidian 友好的写作约定。
